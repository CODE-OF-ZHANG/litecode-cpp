...
| v1.3.3.8 | 2026-07-22 | Phase 5 ★ cookie-aware 修正 POST `/api/v1/auth/logout` 与 Phase 5 cookie refresh token 的不兼容 bug；**隐藏 bug**：前端 `api.auth.logout()` (web/js/api.js:993) 自 Phase 5 (v1.2.21) 起改发空 body `{}` (refresh 已搬到 HttpOnly `lc_refresh` cookie)，但 `parse_logout_request` (auth_routes.h:456) 仍强制 body 必带 `refresh_token` → 生产 logout 永远 400，`clear_refresh_cookie` 因早 return 不被执行 → `Set-Cookie Max-Age=0` 不下发，refresh 也**永远不进黑名单**；**修复** (src/routes/auth_routes.h)：`LogoutRequest::refresh_token` 改 `std::optional<std::string>`；`parse_logout_request` 接受缺失字段（返回 `nullopt` + 不写 envelope，present-but-non-string / present-but-空串 仍 400）；新增 `extract_refresh_token(req, cfg, std::optional<std::string>)` 重载让 logout 复用 refresh 的 cookie-first 优先级；`logout_handler` 流程重排：(1) `require_authentication` → (2) `detail::clear_refresh_cookie(res)` **立即**放 Bearer 鉴权后（pre-v1.3.3.8 的 clear 在 body parse 后，400 路径永远漏 clear，XSS-steal 窗口打开）→ (3) 仅在 `req.body` 非空时 `parse_json_body` + `parse_logout_request` → (4) `extract_refresh_token` cookie 优先 → (5) `revoke_refresh_token` best-effort（空 refresh 走 no-op, revoked=false）→ (6) `send_success({logged_out, revoked})`；`extract_refresh_token` 重载语义与 refresh 完全一致：cookie 优先 → `allow_body_fallback` 才看 body。**测试** (tests/unit/test_auth_logout.cpp)：翻转 `RejectsMissingRefreshToken` / `RejectsEmptyBody` → `LogoutAcceptsMissingRefreshToken` / `LogoutAcceptsEmptyBody` (200 + revoked=false)；`RejectsNonStringRefreshToken` / `RejectsEmptyRefreshToken` / `RejectsMalformedJson` 保留 400 但**加 Set-Cookie Max-Age=0 断言** 锁契约"400 路径仍清 cookie"；parser unit `RejectsMissingField` → `AcceptsMissingField` + 新增 `AcceptsMissingFieldWithSurplusKeys`；新增端到端 case `LogoutRevokesCookieRefresh` (login → Cookie: lc_refresh=... → 200, revoked=true, jti 入 blacklist, 复用同 cookie /auth/refresh 返 401) + `LogoutWithoutAnyTokenClearsCookie` (无 cookie 无 body, 200 + revoked=false, Set-Cookie 仍下发)；tests/unit/test_auth_cookie_storage.cpp `LogoutClearsCookieWithMaxAgeZero` 强化：login + 真发 cookie + 断言 revoked=true + jti 进 store + 复用 401；新增 `LogoutClearsCookieEvenWithoutCookiePresented` 锁"无 cookie 也下发 clear-cookie"防御契约。**SPEC 同步**：§5.1 表格行 / §11 Phase 2 ★ Logout API 详情段 重写。**前端零改动** —— `app.js:332` 的「退出登录」按钮 + `api.auth.logout()` (api.js:993) 自 v1.2.21 起已正确发空 body，后端修好即跑通。**不修**：access token 黑名单 / `users.token_version` Claims 字段 / admin update_role 自增 (JWT role caching 2h-7d 窗口，已在 memory `project-v1-3-3-7-admin-tool-and-jwt-cache.md` 标记 v1.3.4 独立 PR) |
| v1.3.4 PR 6 | 2026-07-24 | **前端深 / 浅模式修复 PR 6** —— 修 3 个用户报告 bug：(1) 浅色模式只剩黑白：style.css:54 处 **CSS comment 漏写 `/*` 开头**（`[hidden]` rule 之后那段 verdict role token map 注释块，开头是 `===========`，结尾是 `*/`；CSS 解析器把 `*/` 当成 stray token 抛错并跳过 `:root { --lc-color-bg, --lc-color-text, --lc-color-link ... }` 整段 token 声明）；后果是浏览器拿到的是"空 `--lc-color-*` + `--lc-v4-*`（第二个 :root 在 4660 行正常加载）"的混合，body bg/color = transparent/rgb(0,0,0)、卡片 / 边框 / 链接全部"消失"，题库 / 排行榜 / 个人主页 / 管理后台一片白底黑字（用户原话"只剩黑白,部分样式消失"）；**修法** = style.css:54 在 === 之前补 `/*` 即可。(2) **代码编辑器 light 模式无高亮 + dark 模式黑字 + 提交按钮看不见**：web/js/editor.js isDarkMode() 同时检查 `html.dark` OR `body.lc-cyber`，但 problem.html 永远 `<body class="lc-cyber">` → `isDarkMode()` 永远 true → Ace 永远用 `tomorrow_night` + 永远挂 `lc-ace--dark` host class；浅色模式下编辑器 bg 是 `var(--lc-editor-bg)` 在 `:root` 是 `#FAF7EE`（燕麦卡其），但 Ace 主题色板是暗色 (`tomorrow_night`)，浅底 + 暗前景 → 关键字 / 字符串 / 数字全部"消失"（用户原话"浅色模式代码没有高亮"）；**修法** = editor.js:174-185 isDarkMode() 只看 `html.dark`，不再误读 `lc-cyber` body 为深色信号（`lc-cyber` 是装饰类,不是 mode 信号）。(3) **`.lc-cyber-podium / .lc-cyber-card / .lc-rank-row*` 全部锁在 `html.dark` 块**：ranking 页 light 模式下前三名 podium 1/2/3 卡片 + rank 表格行 = 0 样式（结构 / bg / border / padding 都没有），排行榜就是"无 podium + 无边框表格"；**修法** = 把结构规则（grid / padding / border / bg `var(--lc-cyber-surface)` / hover / me-row 高亮）从 `html.dark` 前置到无条件 selector（token cascade 通过 `body:not(.dark).lc-cyber` 自动 remap paper 配色）；深空赛博专属的霓虹 / glow / text-shadow 仍锁在 `html.dark` 内。**新增 polish**：podium 1/2/3 在 light 用 paper-tuned 焦糖 / 浅墨 / 焦糖配色，hover `translateY(-2px)`；rank-row hover 用 `var(--lc-cyber-bg-2)`（燕麦 card hover 色）；rank-me-tag bg/contrast 反转（cyan bg + `var(--lc-cyber-bg)` 文字，两套主题都对得上对比）。**零后端改动**，纯 web/。**验证**：agent-browser 截图 5 页 × 2 主题 = problems_list / ranking / profile / admin/{dashboard,users,problems,problem-edit,audit-logs} / problem(代码编辑器) light+dark 全部正常 — light = 燕麦暖白 + 草绿 CTA + 焦糖 hover + 草绿 link；dark = 深空赛博 + 信号青 CTA + 霓虹 glow；编辑器 light = github 主题 + 燕麦底；dark = tomorrow_night + 深空底；提交按钮 / 运行样例 按钮 light = 草绿 / 透明草绿边；dark = 青→紫渐变 / 透明青边。**踩坑**：之前 PR 4 加 `.lc-ace--dark / .lc-ace--leetcode-light` host class 是对的，bug 只是 isDarkMode() 误读了 `lc-cyber`；现在保留 host class 机制，只清掉"把 lc-cyber 当中点"那行 `|| body.classList.contains('lc-cyber')`。
...
- [x] ☆ 渗透测试（XSS / SQL 注入 / CSRF 扫描）—— v1.2.66（详见下方 Phase 8 详表 A37–A39）
- [x] △ 模糊测试（fuzzing）判题输入 —— v1.2.67（详见下方 Phase 8 详表 A40–A43）
...
- [x] △ 性能 Profile（`perf` / `flamegraph` 跑判题热路径）
...
> - **v1.3.4 PR 11 ★ super_admin 硬锁定**: 默认 `username='admin'`(V099 种子,user_id=1)是唯一超级管理员,前端 `litecode.auth.isSuperAdmin()` + 后端 `auth_routes.h::update_username_handler` / `admin_user_routes.h::change_user_role_handler` 双层防御: (1) admin 改自己 username → 400 INVALID_INPUT reason=`super_admin_locked`;(2) 普通用户改名为 admin → 409 CONFLICT reason=`username_reserved`;(3) admin 把自己 role 改为 user → 403 FORBIDDEN reason=`admin_self_demote_forbidden`。profile.html 编辑模态隐藏改 username section + 显示 🔒 banner;admin/users.html super_admin 行加 `.lc-user-row--super-admin` 渐变 + `.lc-locked-pill` 锁定 pill
> - **v1.3.4 PR 12 ★ 测试残留账号过滤**: 排行榜 `/api/v1/stats/ranking` 后端 `list_leaderboard` + `count_ranked_users` SQL 加 `WHERE username NOT REGEXP '^(e2e_|lc_perf_|normaluser_|stress_|load_|fuzz_)'`(配合 `scripts/purge_users.sh --yes` 一次性清掉历史 93 个 e2e_* / lc_perf_* / normaluser_test 残留 → users 表只剩 admin + zhangxu);前端 `web/ranking.html` `renderRows` 入口用同一正则再过滤一遍 + 重新 1-based 连续编号 rank(防止过滤后排名留空档)。**保留名单**:`admin` (V099 唯一 super_admin) + `zhangxu` (项目 owner);其余均视作测试残留。配套 SQL 迁移 `db/migrations/V016__clean_e2e_and_perf_users.sql`(idempotent)。
>
> **密码策略**:
...
| PUT  | `/api/v1/auth/username` | 已登录 | 5 次/小时/用户 | **v1.3.4 PR 9** + **PR 11 增强**:改 username(1 天 1 次冷却,429 + Retry-After);**PR 11** 加 reserved 黑名单:`username='admin'` 是系统保留用户名,任何用户改名为 admin → 409 CONFLICT reason=`username_reserved`;admin 自己改 username → 400 INVALID_INPUT reason=`super_admin_locked`(只锁 username,display_name / school / bio / email / avatar 仍可改) |
...
| GET | `/api/v1/stats/ranking` | 公开 | 30 次/分/IP | 排行榜（默认 100 名，可选分页）;**v1.3.4 PR 12** 加测试残留账号过滤:`list_leaderboard` 与 `count_ranked_users` SQL 都加 `WHERE username NOT REGEXP '^(e2e_\|lc_perf_\|normaluser_\|stress_\|load_\|fuzz_)'` —— scripts/e2e_acceptance.sh / load_test.sh / fuzz_judge.sh 持续通过 `/api/v1/auth/register` 写测试残留(`e2e_u_*` / `e2e_rl_reg_*` / `lc_perf_*` / `normaluser_test` 等),V015 只覆盖 ^test_/^demo_/^bot_ 不够,深度防御在 SQL 层兜住;前端 `web/ranking.html` `renderRows` 入口再用同名正则再过滤一遍 + 重新连续编号 rank;**v1.3.4 PR 12 follow-up** 排行榜页面增宽:`web/css/style.css` 新增 `.lc-rank-page`(max-width 1440px)+ 行高 64px + 列宽 `80/1fr/120/120/140` + row hover `translateX(4px)` + rank 22px + solved 20px 主指标 + podium rank 36→44 |
...
| PUT | `/api/v1/admin/users/:id/role` | 🔒 admin | 10 次/分 | 修改用户角色（写入 `audit_logs`）;**v1.3.4 PR 11** 加 self-demote 403:admin 把自己 role 改为 user → 403 FORBIDDEN reason=`admin_self_demote_forbidden`(只允许 admin 间互提权 + admin 降自己以外的用户);前端 `/admin/users.html` super_admin 行(默认 `username='admin'`)+ 自己的行都不显示降级按钮,显示 🔒 锁定 pill |
...
| 页面 | 路径 | 权限 | 说明 |
|------|------|------|------|
| 首页（落地页） | `/` | 公开 | Hero + 6 状态卡 + 适用场景 + 使用流程（v1.3.3.9 起不再承担题单） |
| 题库列表 | `/problems_list.html` | 公开 | 题目列表，筛选、搜索、分页（v1.3.3.9 从 `/` 拆分） |
| 刷题页 | `/problems/:slug` | 公开 | 双栏：左题目右编辑器，**代码草稿自动保存到 localStorage**，**编辑器 v1.3.3.9 起使用 Ace 1.32.7** |
| 登录页 | `/login` | 公开 | 用户登录 |
...
| **super_admin 锁定 + 视觉升级** (v1.3.4 PR 11) | (1) `username='admin'` 硬锁定(改 username 拒 + 改自己 role 拒 + 别人改名为 admin 拒);(2) 5 个 admin 页装饰统一(原 0 装饰→ 4 角 SVG + grid-strip + bg-oat + bg-aurora + bg-grid + spotlight + watermark);(3) 导航栏毛玻璃 + 修深色"上空一大片"根因(`.lc-cyber-grid-strip` 改 fixed + z-index:51);(4) Ace 编辑器极客色(bg `#1a1b3a` + keyword `#8ab4ff` + string `#a8e067` + number `#ffb088` + function `#80d8ff` + comment italic opacity 0.85);(5) Canvas 鼠标粒子层(mouse-deco 写的 `--lc-spotlight-x/y` 复用 rAF,35 个粒子 + 连线 + 鼠标引力);(6) profile 4 KPI 仪表盘(SVG progress ring + linearGradient fill);(7) ranking 每行通过率 mini-bar + podium 加大(160→220px);(8) 侧边几何网格 `.lc-v4-bg-grid` z-index:-2 | 后端 `db/migrations/V015__clean_test_users.sql`(清洗 test_/demo_/bot_ 前缀历史假人);`src/routes/auth_routes.h::update_username_handler` 在 validate_username 之后加 admin lock + reserved 黑名单;`src/routes/admin_user_routes.h::change_user_role_handler` 在 require_admin 之后加 self-demote 403。前端 `web/js/api.js` 加 `litecode.auth.isSuperAdmin()`(只看 username);`web/js/mouse-particles.js`(新建,245 行 Canvas 真粒子);`web/css/style.css` §6.6 grid-strip fixed fix + §新加 `[data-nav]` backdrop-filter + §新加 Ace token 精修 + §新加 `.lc-v4-kpi-card` + `.lc-v4-bg-grid` + `.lc-rank-row__rate-bar` + `.lc-banner--locked` + `.lc-locked-pill` + `.lc-user-row--super-admin`;`web/profile.html` 加 svg defs 共享渐变 + `kpiCard()` 函数 + `openEditProfile` super_admin 锁定分支;`web/admin/users.html` `renderRow` 加 isSuperAdmin 检测 + role pill 改"超级管理员 🔒" + actions 列替换为 🔒 锁定 pill;`web/ranking.html` 每行加 `.lc-rank-row__rate-bar` + `--rate` 变量;5 admin HTML + 4 public HTML(login/register/problem/profile)补 `<canvas id="lc-particles-canvas">` + `<script src="/js/mouse-particles.js?v=1.3.5">` |
| **4 bug 二次收口** (v1.3.4 PR 12) | (1) 导航栏上空彻底清空(`.lc-cyber-grid-strip` 双模式 `display: none` + `html.dark body > *:not(...)` 排除列表扩展,根因:PR11 的 specific `(0,1,0)` 败给 `body > *` `(0,1,2)` 被强制 relative,2px 渐变条顶回文档流);(2) Canvas 鼠标粒子双套 CONFIG(dark N=70/引力 260/连线 150,glow 10×;light N=55/引力 220/连线 130,glow 9×,PR11 共用 35/160/90 → 现在两边都多一点 + 动幅度大一点),3 个公开页面(index/ranking/problems_list)补齐 canvas+script,profile 升 1.3.5,修 DPR 累积 bug(`setTransform` 替代 `scale`);(3) 排行榜测试残留 93 个账号清洗:`scripts/purge_users.sh --yes` 一次性清完 + `db/migrations/V016__clean_e2e_and_perf_users.sql`(idempotent)+ 后端 `stats_routes.h::list_leaderboard`+`count_ranked_users` SQL 加 `WHERE username NOT REGEXP '^(e2e_\|lc_perf_\|normaluser_\|stress_\|load_\|fuzz_)'` + 前端 `web/ranking.html::renderRows` 入口再过滤 + 重新连续 1-based 编号 rank(用户原话"只需要 zhangxu 和 admin 两个账号");(4) problem.html 编辑器往右贴边(类 LeetCode):`.lc-problem-page` `max-width:none !important` 去掉 1480 限制 + `padding: 0 0 96px 32px`(padding-right:0),`.lc-problem-layout` grid-template-columns `5fr 7fr`(题目 42% / 编辑器 58%),`.lc-editor-pane` `padding-right: 32px`(卡片内容不贴物理边)+ 响应式断点同步缩窄/归零 | 后端 `src/routes/stats_routes.h` `list_leaderboard` 与 `count_ranked_users` 都加 WHERE 过滤(深度防御,e2e 脚本持续写测试账号,SQL 层兜住);新建 `db/migrations/V016__clean_e2e_and_perf_users.sql`(INSERT IGNORE schema_migrations)。前端 `web/css/style.css` §6.6 `.lc-cyber-grid-strip` 改 display:none universal + L6043 `:not()` 排除列表扩到 10 类 fixed 装饰(含 `.lc-cyber-grid-strip` / `.lc-v4-bg-{oat,aurora,grid}` / `.lc-v4-spotlight` / `.lc-v4-deco` / `.lc-v4-watermark` / `.lc-svg-defs` / `#lc-particles-canvas`)+ `.lc-problem-page` 移动 padding-right 到 `.lc-editor-pane` 自带 + `.lc-problem-layout` 改 5fr 7fr;`web/js/mouse-particles.js` 升级:`CONFIG.{dark,light}` 双套参数 + 主题切换时检测 `appliedCfg !== cfg` 重建粒子 + `ctx.setTransform(dpr,0,0,dpr,0,0)` 替代 `ctx.scale`(修 DPR 累积);`web/index.html` / `web/problems_list.html` / `web/ranking.html` `</body>` 前补 `<canvas id="lc-particles-canvas">` + `<script src="/js/mouse-particles.js?v=1.3.5">` + 其他 script version bump 1.3.4→1.3.5;`web/profile.html` mouse-deco 同步升 1.3.5;`web/ranking.html::renderRows` 入口加 `isVisibleRankingItem()` filter + `state.total = visibleItems.length` + 重新 `it.rank = i + 1` 连续编号 |
| **3 项视觉/可用性收口** (v1.3.4 PR 12 follow-up) | (1) 浅色模式 Canvas 粒子可见性修复:`#lc-particles-canvas` z-index 从 -1 升到 0(浅色下被 `.lc-v4-bg-oat` 不透明燕麦色 `#F8F6F0` 同层遮住,升 z 让 pointer-events:none 的固定粒子层浮于装饰层之上)+ 浅色专属 `mix-blend-mode: multiply` 让粒子颜色 * 燕麦底 = 自然加深;深色下 normal 不变。光斑在燕麦底上对对比强烈可见。(2) 排行榜页面增宽:新增 `.lc-rank-page` modifier(max-width 1180→1440px + 40px padding),grid 列宽 `64/1fr/92/92/104` → `80/1fr/120/120/140`,rank row min-height 提升到 64px,`.lc-rank-cell--rank` 字号 18→22,`.lc-rank-cell--solved` 单独提至 20px(主指标色 + link token 突出),podium rank 字号 36→44,响应式断点 ≤1280px 回退 1180px。(3) 题库难度筛选居中丰富:行结构从单行 flex 改 column — 第一行 heading(难度标签 + 副标题 hint)+ 第二行 4 个居中按钮(三段式布局 图标 + 文字 + 计数徽章);按钮配色用 sem 色条:全部 📚 / 简单 🌶 + 易绿 `#10b981` / 中等 ⚡ + 琥珀 `#f59e0b` / 困难 🔥 + 危险红 `#ef4444`,active 态用 `linear-gradient(135deg, link, link+indigo)` + `box-shadow` 内嵌白高光;后端无 per-difficulty count 接口,前端 `loadDifficultyCounts()` 拉一次 `?limit=200` 全量 + 按 difficulty 分桶,缓存到 `difficultyCounts`,`applyDifficultyCounts()` 把数字写入 `.lc-seg__btn-count`;移动端 (≤768px) 4 按钮 2x2 网格。 | 前端 `web/css/style.css` §17 `.lc-rank-page` + `.lc-rank-table` 列宽/字号/padding/hover transform `translateX(4px)`;§5 lc-cyber-podium rank/name/solved 字号全提一档;§8 lc-problem-filters 子块:新增 `.lc-problem-filters__row--difficulty`(column 布局 + padding)+ `.lc-problem-filters__heading` + `.lc-problem-filters__heading-hint`(JetBrains Mono)+ `.lc-seg--centered`(flex-1 等分 + 4px padding + shadow)+ `.lc-seg__btn--active` `linear-gradient` + `box-shadow` 内嵌 + `--lc-pill-{easy,medium,hard}` 色条 + 移动端 grid 2x2;`#lc-particles-canvas` z-index:-1→0 + `mix-blend-mode: normal` + 浅色 `multiply` + opacity 0.92。`web/js/mouse-particles.js` `PALETTE.light` 4 色 lightness 45→38 + saturation 70→85%(multiply 后仍鲜明)+ `CONFIG.light` N 55→75 + mouseInfluence 220→240 + lineDist 130→160 + force 0.020→0.026 + drift 0.20→0.26 + sizeMin 2.0→2.6 + sizeMax 4.2→5.2 + nearSizeMul 2.0→2.4 + glowMul 9→11 + nearR 80→95(浅色下比深色更激进,补偿 multiply 视觉衰减)。`web/ranking.html` line 73 `<main>` class 加 `lc-rank-page`。`web/problems_list.html` `renderDifficultySeg` 重写为三段式(icons + text + count span)+ 新增 `loadDifficultyCounts()` + `applyDifficultyCounts()` + DOMContentLoaded 挂载点。 |
| **深色模式装饰补齐 + Canvas 粒子全局克制化** (v1.3.4 PR 12 follow-up-2) | (1) 用户报"深色模式 `.lc-problem-filters__row` 没丰富" → 浅色 block 在 `body.lc-cyber` 不继承,补整段深色专属样式:`.lc-seg--centered` 容器用暗紫蓝渐变 + cyan border + glow box-shadow(同 `.lc-v4-kpi-card` 质感);hover 改 cyan→indigo 渐变 + glow 反馈;active 改 cyan→violet 浓烈渐变 + 双层 glow(外 cyan + 内 violet)+ inset 白高光 + 计数徽章翻深;`.lc-problem-filters__row--difficulty` 容器加暗紫蓝渐变 + cyan 顶边线 + `::before` 顶部 cyan 强调线;`.lc-problem-filters__heading` label 加 cyan text-shadow + hint 改 cyan 透明体;`.lc-problem-list` 容器加同款暗紫蓝渐变 + cyan 边光;`.lc-problem-row:hover` 改 cyan→indigo 渐变 + 2px cyan 左侧色条(避免过滤区惊艳反馈到列表区"没反应"的断层)。(2) 用户报"Canvas 粒子颜色深,遮挡题目等文字" → 整粒子系统"克制化":`globalCompositeOperation` `'lighter'` → `'source-over'`(重叠不再爆光,alpha 上限 1.0);`PALETTE.dark` 4 色 lightness 65-70% → 75-82% + saturation 70-90% → 55-72%(深空底上更接近星星点点,降饱和不抢戏);`PALETTE.light` 4 色 lightness 35-48% → 55-65%(提亮补 multiply 视觉衰减,不再走"更暗更饱和"反向逻辑);`CONFIG.{dark,light}` 整体收:N 70/55 → 55/60、mouseInfluence 260/240 → 220/210、lineDist 150/160 → 130/140、force 0.024/0.026 → 0.020/0.022、drift 0.24/0.26 → 0.20/0.22、spring 0.0014 → 0.0016(更快归位)、sizeMin/Max 2.5-5.0/2.6-5.2 → 2.0-4.0/2.1-4.2、nearSizeMul 2.2/2.4 → 1.7/1.8、glowMul 10/11 → 6/7、nearR 80/95 → 70/75;粒子本体 alpha 基础 0.6 → 0.32、鼠标附近 0.95 → 0.55;连线 alpha 0.35 → 0.18,nearBoost 0.5 → 0.25;glow alpha 0.4 → 0.22。 | 前端 `web/css/style.css` §8 新增 5 段 `html.dark .lc-seg--centered` 系列(容器 / btn / hover / active / count / 3 个 difficulty 色条)+ §8 新增 `html.dark .lc-problem-filters__row--difficulty` 容器渐变 + `::before` 强调线 + heading label/hint 深色专属 + §8 新增 `html.dark .lc-problem-list` 容器渐变 + `.lc-problem-row:hover` 深色反馈 + 边框透明度。`web/js/mouse-particles.js` `PALETTE.{dark,light}` 全套 lightness/saturation 提档 + `CONFIG.{dark,light}` 全参数收一档 + tick() `globalCompositeOperation` 改 `source-over` + 连线 alpha 系数 0.35→0.18 + nearBoost 0.5→0.25 + 粒子本体 alpha 基础 0.6→0.32 / 近鼠 0.95→0.55 + glow alpha 0.4→0.22。 |
| **5 bug 三方收口 + 粒子 scoped 化** (v1.3.4 PR 13) | (1) **4 个 section 暗色补全粒子** — 原 viewport 全屏 `#lc-particles-canvas` 被 `.lc-v4-hero` / `.lc-problem-filters` / `.lc-rank-table` / `#profile-content` 这 4 个带 opaque 背景的容器直接挡住(z-index:0 canvas 在卡片背景之后),深色下完全看不到。修法:每个容器内嵌 `<canvas class="lc-particles-canvas-scoped">`(绝对定位 inset:0 + z-index:0 + pointer-events:none),`.lc-particles-host` helper 一并加 position:relative + overflow:hidden + isolation:isolate。`web/js/mouse-particles.js` 重写支持多 canvas:每个 canvas 独立 measure/resize/粒子系统,鼠标全局共享,粒子在容器局部坐标系内活动。scoped 配置更克制:N 22/26 + alphaBase 0.18 + alphaNear 0.30 + glowMul 0(无发光,绝不影响阅读);浅色 multiply 沿用;深色 normal(同全局 canvas)。(2) **暗色 modal 弹窗跑到页面底部** — 根因:`html.dark body > *:not(...) { position: relative; z-index: 2 }` 规则覆盖了 `.lc-modal` 的 `position: fixed; z-index: 50`,导致 modal 退化成 `position: relative` 留在文档流末尾(页面所有内容之后)。修法:`.lc-modal` 加到 `:not()` 排除列表(其他 11 类 fixed 装饰不变),modal 保持 fixed 居中。(3) **改头像后离开再回来失效** — 根因:`profile.html::renderHeader()` 无条件 `$('avatar').textContent = 'X'`,把任何已渲染的 `<img>` 整个删掉,reload 后只显示首字母。修法:抽 `applyAvatarToElement(el, avatarUrl, fallbackChar)` helper(有 URL 建/复用 img + cache-bust,没 URL 清 img 回退文字),renderHeader / openEditProfile / processAvatarFile / save handler 共用。(4) **改头像后右上角 nav 不更新** — 根因:`web/js/app.js::renderNav` 永远只渲染 `<span class="lc-avatar">X</span>`(纯文字),即便 `user.avatar` 有 URL 也不会出 `<img>`;profile.html save handler 也只更新 `#avatar` 没找 nav 元素。修法:renderNav 检查 `user.avatar` 渲染 `<img>`(width/height 100% + object-fit:cover);profile.html save handler 找到 `[data-nav] .lc-nav-user > .lc-avatar` 调 `applyAvatarToElement`,**同时手动写 `sessionStorage['litecode:user']`**(api.js::uploadAvatar 走单独端点不更新缓存,跨页 boot.shell 读到的 user 永远是旧头像)。(5) **方形头像显示成方框** — 根因:`.lc-v4-avatar-xl` 与 `.lc-avatar` 有 `border-radius: 50%` 但**没有 overflow:hidden**,子 `<img>` 仍按方形渲染 + 溢出圆角边界。修法:两处都加 `overflow: hidden`。**(6) 排行榜"我"那行头像** — 后端 `stats_routes.h::list_leaderboard` 故意 omit 其他用户 avatar(confidentiality 注释),但用户上传头像后**自己那一行**仍显示首字母不像预期。修法:`web/ranking.html::normalizeRankItem` 保留 `avatar` 字段(预留后端放开时的零改动启用)+ `renderRows` 在 `isMe` 分支用 `auth.currentUser.avatar` 拼上行 avatar(后端零改动,只"我"那行支持)。**(7) 跳页 nav 头像闪动跳默认再恢复** — `web/js/app.js::hydrateUser` 与 `bootShell` 的 `auth-changed` listener 在 fetchProfile 完成后会触发 1-2 次 renderNav,如果 server user 的关键字段(avatar/username/role)跟 cachedUser 一致,DOM 重建会把刚加载完的 `<img>` 拆掉换首字母再换回来,用户看到 flicker。修法:两次 renderNav 入口前加 sameDisplay 比较(三个字段一致就只更新 `navState.user` 引用、不调 renderNav)。 | 前端 `web/css/style.css` §6.3 末尾新增 `.lc-particles-host`(position:relative + overflow:hidden + isolation:isolate)+ `.lc-particles-canvas-scoped`(position:absolute + inset:0 + z-index:0 + pointer-events:none + opacity:0.85 + mix-blend-mode normal / 浅色 multiply);L6384 `html.dark body > *:not(...)` 排除列表扩到 11 类(`.lc-modal` 加入);§9a `.lc-v4-avatar-xl` + §7 `.lc-avatar` 各加 `overflow: hidden`。`web/js/mouse-particles.js` 重写为多 canvas 系统:discoverCanvases(全局 + scoped)+ measureAll + per-canvas initEntryParticles + tick 内 `ensureEntryCfg + renderCanvas`,scoped 局部坐标换算(scroll listener 同步 hostLeft/hostTop)+ ResizeObserver 监听 scoped host 重测,#profile-content 从 hidden→visible 时自动重置粒子;SCOPED_CONFIG 双套深/浅独立参数(粒子数 / 引力 / 连线 / 大小 / alpha / glowMul 全独立)。`web/index.html` / `web/problems_list.html` / `web/ranking.html` / `web/profile.html` 4 个目标容器加 `lc-particles-host` class + 内嵌 `<canvas class="lc-particles-canvas-scoped">`。`web/profile.html` 抽 `applyAvatarToElement()` helper(放在 renderHeader 之前)+ `renderHeader(user)` 改调 helper(替代无条件 textContent)+ `openEditProfile()` preview + `processAvatarFile()` preview + save handler 调 helper 同步 `#avatar` + `[data-nav] .lc-nav-user > .lc-avatar` + 手动 `sessionStorage.setItem('litecode:user', JSON.stringify(cachedUser))`(uploadAvatar 单独端点不写 cache 的兜底)。`web/js/app.js::renderNav` avatar 元素生成逻辑分叉:`user.avatar` 有 URL 走 `<img>`(100% + object-fit:cover)无 URL 走首字母 textContent;`hydrateUser` + `bootShell` 的 `auth-changed` listener 入口加 sameDisplay 比较(三个字段一致跳过 renderNav)。`web/ranking.html::normalizeRankItem` 保留 `avatar` 字段;`renderRows` 在 `isMe` 分支用 `me.avatar` 拼 rowAvatar,行头像 `<span class="lc-avatar">` 按 rowAvatar 是否为 URL 渲染 `<img>` 或首字母 textContent。 |
| **6 bug 一次性收口** (v1.3.4 PR 13 follow-up) | (1) **Admin dashboard 滚动闪烁 + 主区域空旷** — `[data-nav]` 用 `backdrop-filter: blur(14px)` + `html.dark body { background-attachment: fixed }` 组合,sticky nav 在每次 scroll 都触发整 viewport blur,Chrome 上是已知 scroll judder 源(用户原话"鼠标向下滑闪一下回顶部")。修法:`[data-nav]` 与 `html.dark [data-nav]` 同步去掉 `backdrop-filter` + `-webkit-backdrop-filter`,改用 linear-gradient 半透渐变(rgba 0.94→0.82),深色用 `--lc-cyber-bg` 同款,scroll 性能肉眼可见地稳。**主区域充实**:`.lc-v4-page-header__title` clamp 上限 38→46px + 中段 4vw→4.5vw,`.lc-v4-page-header__lede` 15→17px + line-height 1.55→1.65(全站生效)。dashboard.html 在 `<header>` 后新增 `<section class="lc-v4-trend-row">`(3 个 cyber-cyan chip:实时 QPS / 在线用户 / 提交分布,spark 动画 `mask-image` + `lc-trend-glide` keyframe 跑 3.6s)+ `<section class="lc-v4-quick-actions">` grid(`repeat(auto-fit, minmax(220px, 1fr))` + gap 18px),4 卡片入口:📚 题库管理 / ⬆️ 批量导入 / 🛡 审计日志 / 👥 用户管理,每张 icon 28px + title 17px/700 + hint 13px/--lc-v4-slate-500;hover translateY(-2px) + border-color link token + cyan 系 box-shadow;深色版用 `--lc-cyber-surface` 渐变 + border 用 `--lc-cyber-border`,title `var(--lc-cyber-text)` + hint `--lc-cyber-text-muted`。 (2) **Admin 题目管理"编辑"和"新建"完全相同** — 根因:`web/admin/problem-edit.html` 中 `$('submit-btn')` 取到 `null`(PR11 重构时按钮从 toolbar 搬走但漏加 header toolbar div),`applyHeaderForMode()` 在 line 730 `submitBtn.textContent = '...'` 抛 TypeError,后续 `wireEvents()` / `loadForEdit()` 全部 unreachable,结果是编辑模式 form 永远不 prefill + 保存按钮触发浏览器 native GET(`<form novalidate>` 没 action/method → URL 自动带参数 = GET,刷新后被误判成 edit 模式)。修法:在 `<header>` 末尾 `</p>` 之后加 `<div class="lc-v4-page-header__row">`(同 dashboard 同款),内嵌 `<div class="lc-v4-page-header__toolbar">` flex 12px gap,放 `<a href="/admin/problems.html" class="lc-v4-btn lc-v4-btn--ghost">取消</a>` + `<button type="submit" form="problem-form" class="lc-v4-btn lc-v4-btn--primary" id="submit-btn">保存</button>`,与 admin/problems.html:73-83 toolbar 同款,使 `$('submit-btn')` 取到非 null。同时更新 line 73-77 过时注释说"toolbar 已搬 v4-page-header__toolbar 但丢了",标 PR13 ★ 已恢复。 (3) **登录/注册暗色模式 1.02:1 对比度 + 无主题切换 + Canvas 粒子被遮** — 三连错叠加:(a) `.lc-v4-auth__pane` 硬编码 `background: #f8fafc` (`style.css:5651`),深色模式下 `#F1F5F9` 文字 `#F1F5F9` 全部近乎不可见(用户原话"网页颜色与文字颜色相近,看不清");(b) `<body class="lc-cyber">` 但没 `[data-nav]` → `app.js renderNav()` 早返回 → 没有主题切换按钮;(c) `.lc-v4-auth` z-index 2 + 不透明 → viewport-fixed `#lc-particles-canvas` (`style.css:8398`) 被整片覆盖,粒子看不见。修法:①CSS 新增 `§23.1` 鉴权深色玻璃拟态:`html.dark .lc-v4-auth__pane { background: rgba(17,24,39,.78); backdrop-filter: blur(20px); border:1px solid rgba(125,211,252,.18); }` + `html.dark .lc-v4-auth__input` 用 `rgba(11,15,26,.55)` bg + `--lc-v4-slate-100` 文字 + cyan focus 边;`html.dark .lc-v4-auth__label / __form-sub / __field-help / __alt` 改 `--lc-v4-slate-200/300`;`body:not(.dark) .lc-cyber-terminal / .lc-cyber-ribbon { display:none }`(`style.css:371-372`)保留,浅色下左侧装饰条隐藏是产品意图不动。②login.html / register.html 在 `.lc-v4-auth__pane` 顶部新增 `<canvas class="lc-particles-canvas-scoped">` + `<button class="lc-icon-btn lc-v4-auth__theme-toggle" data-act="theme-toggle" aria-label="切换深色模式">🌗</button>`(CSS §23.2 绝对定位 top:20/right:20/z:4);theme-color meta 改 `content="#0A0E1A"` 与默认深色一致。③在 login/register 内联 IIFE 末尾(`/js/mouse-particles.js` 之前)挂 click listener:`document.querySelector('[data-act="theme-toggle"]')?.addEventListener('click', () => litecode.theme.toggle())` + 派一次伪 `litecode:theme-changed` 事件让 `app.js syncThemeButton()` 同步初始图标(`syncThemeButton()` 监听 `litecode:theme-changed` 全站,只靠它无需 nav 也能工作)。script `?v=1.3.4` → `1.3.5` 同步。  (4) **排行榜榜框过窄 + 排名/用户名/已解决字号不一致** — 现状:列宽 `80/1fr/120/120/140`(`style.css:2959`)+ `.lc-v4-card` inline `padding:0;overflow:hidden`(`ranking.html:134`)让卡片紧贴容器边缘 + 没外框;font 号 rank 22 / solved 20 / num 18 / username 16 / header 13 / 我 tag 12 完全不一致;深色 cyber 块额外有 letter-spacing 0.08em 让头部更散。修法:①CSS 新增 `.lc-rank-card-frame`(padding `clamp(20px, 3vw, 36px)` + border 1px + border-radius + background linear-gradient + shadow);`html.dark .lc-rank-card-frame` 用 `--lc-cyber-surface` 渐变 + cyan glow `box-shadow: 0 0 0 1px rgba(34,211,238,.12), 0 8px 24px -12px rgba(34,211,238,.25)`。②列宽 `80/1fr/120/120/140` → `96/1fr/132/132/156`(全加 16px)。③统一字号梯队:`.lc-rank-head` 13→15px + letter-spacing 0.06→0.04em;`.lc-rank-cell--rank` 22→24px + medal 26→28px;`.lc-rank-username` 16→19px + font-weight 600;`.lc-rank-cell--num` 18→20px + font-weight 700;`.lc-rank-cell--solved` 20→22px + font-weight 800(主指标仍最大,但与 rank 差从 4px 缩到 2px);`.lc-rank-me-tag` 12→13px + padding `2px 8px`→`3px 10px`(用户报"我字有时不显示",靠加大 pill 视觉重量弥补)。④深色 cyber 块 `.lc-rank-head` 11→13px + letter-spacing 0.08→0.05em + `text-shadow: 0 0 12px rgba(34,211,238,.35)` 让深色模式下头部字也"发光"。⑤ranking.html line 134 改 `<section class="lc-v4-card lc-rank-card-frame lc-fade-in" id="table-state" hidden>` 删 inline padding:0/overflow:hidden。 (5) **排行榜"0 / 6" + "我"标首次刷新丢失** — 两独立 bug 一次性收口:**A** 后端 `stats_routes.h::serialize_leaderboard_row` 不发 `accepted_count`(PR11 漏字段),前端 `ranking.html:435` `it.accepted + ' / ' + it.submissions` 左侧永远 0。修法:`LeaderboardRow`(`stats_routes.h:480`) 新增 `int accepted_count = 0`;SQL aggregate `list_leaderboard` 加 `COUNT(CASE WHEN s.status = 'ac' AND p.is_deleted = FALSE THEN 1 END) AS accepted_count`,row[5] 解析;`serialize_leaderboard_row`(`stats_routes.h:785`) 加 `{"accepted_count", r.accepted_count}`;前端 `normalizeRankItem` `pickNum(raw, ['accepted','accepted_count','ac_count'])` 已能识别,**零改动生效**。**B** `ranking.html:368 renderRows` 同步读 `litecode.api.auth.currentUser`,但 boot.shell 是异步,首次加载时 currentUser 还没 hydrate → myName=null → 无行被标 isMe → "我"标看不见,用户"点几下才出来"是因为 paginate 重渲染触发,boot 已完成。修法:①`window.addEventListener('DOMContentLoaded')` 把 `litecode.boot.shell({...})` 链 `.then(start, start)`,start 内才挂 retry-btn / popstate / 调 `load()`,保证首屏在 currentUser hydrate 之后跑。②`renderRows` 之上加 localStorage 兜底:`var me = auth.currentUser || JSON.parse(localStorage.getItem('litecode:user') || 'null')`(STORAGE_KEY_USER = 'litecode:user' 见 `api.js:98`),双保险。 (6) **profile 暗色模式无 Canvas 真实粒子** — 根因:`web/profile.html` 同时加载 2 份 `mouse-particles.js`(行 363-364 + 行 1377-1378,后者紧随 inline IIFE 之后的全局 canvas),2 份 IIFE 都跑 `discoverCanvases()` → 各自建一套粒子数组 + 各自 rAF tick + 各自 `clearRect`,结果每帧一个抹另一个的画布,视觉上像"没粒子"(明明存在但被自己覆盖)。修法:删 `profile.html` 全局 canvas 旁的重复 `<script src="/js/mouse-particles.js?v=1.3.5" defer>`,只保留中段 `<head>` 链头的那一份(与其他 4 个 public HTML 一致)。`#profile-content.lc-particles-host` + 内嵌 `<canvas class="lc-particles-canvas-scoped">` 早已存在,`.lc-particles-host` CSS 也已就位;`mouse-deco.js` + 单份 `mouse-particles.js` 加载后,深/浅色按 CONFIG.dark/light + PALETTE.dark/light 双套自动切换,无需再动粒子引擎。`theme-boot.js?v=1.3.4` 与 `style.css?v=1.3.4` 一并升 1.3.5 与其他页面对齐。 | 后端 `src/routes/stats_routes.h`:`struct LeaderboardRow` 加 `int accepted_count = 0;`(`stats_routes.h:480`);`list_leaderboard` SQL `LEFT JOIN` aggregate 新增 `COUNT(CASE WHEN s.status = 'ac' AND p.is_deleted = FALSE THEN 1 END) AS accepted_count`(`stats_routes.h:556-584`),COALESCE 一列 + `row[5].get<std::int64_t>()` 解析;`serialize_leaderboard_row` 加 `{"accepted_count", r.accepted_count}` 字段 + doc comment 同步加 `accepted_count: 35`。前端 `web/css/style.css`:① §6.1 `[data-nav]` 与 `html.dark [data-nav]` 同步去掉 `backdrop-filter` / `-webkit-backdrop-filter`,改 linear-gradient 半透渐变(浅: `rgba(248,246,240, 0.94→0.82)`;深: `rgba(10,14,26, 0.94→0.78)` + cyan 边沿 0.28 + 上浮阴影)。②新增 `§23` 鉴权深色 + dashboard 趋势 + 快捷操作块:`html.dark .lc-v4-auth__pane` 玻璃拟态 + `html.dark .lc-v4-auth__input` 深底 + `.lc-v4-auth__theme-toggle` 绝对定位 + `.lc-v4-quick-actions` grid + 4 卡片(`.lc-v4-quick-action` 含 __icon/__title/__hint)+ `.lc-v4-trend-row` + `.lc-v4-trend-chip` 含 `__spark` 用 mask-image 实现闪烁扫描条(rAF 不参与,纯 CSS keyframe `lc-trend-glide` 3.6s linear infinite),深色版用 `--lc-cyber-surface / -border / -text`。③ `.lc-rank-card-frame` 块(浅深双版)+ 列宽 96/1fr/132/132/156 + font-size 全梯队提一档(13→15 / 16→19 / 18→20 / 20→22 / 12→13,深色 cyber 11→13 + letter-spacing 0.08→0.05em + text-shadow cyan glow)。④ `.lc-v4-page-header__title` clamp(32px, 4.5vw, 46px)(全 site 影响,值得) + `.lc-v4-page-header__lede` 17px + line-height 1.65。`web/admin/dashboard.html` header 之后加 3 chip + 4 quick-action section。`web/admin/problem-edit.html` header 末尾补 `<div class="lc-v4-page-header__row">` + toolbar div 内含 `取消` link + `保存` button(`id="submit-btn"`)。`web/login.html` + `web/register.html`:`.lc-v4-auth__pane` 顶部 `<canvas class="lc-particles-canvas-scoped">` + `<button data-act="theme-toggle" class="lc-icon-btn lc-v4-auth__theme-toggle">🌗</button>`;IIFE 末尾挂 click → `litecode.theme.toggle()` + 派伪 `litecode:theme-changed` 事件让 syncThemeButton() 同步图标。`web/ranking.html`:line 134 table-state 改 `lc-rank-card-frame lc-fade-in` 删 inline `padding:0/overflow:hidden`;`renderRows` 上方加 meCached localStorage 兜底;`DOMContentLoaded` listener 内 `boot.shell().then(start, start)`,start 内才挂 retry-btn / 调 load(),保证首屏 currentUser 已 hydrate。`web/profile.html`:删行 1378 重复 `<script src="/js/mouse-particles.js?v=1.3.5">`(留中段头链那一份);`theme-boot.js?v=1.3.4` + `style.css?v=1.3.4` → `1.3.5` 与其他页面统一。**rebuild 必须**:本 PR 改 `src/routes/stats_routes.h`,按 memory `rebuild-web-after-backend-edit` 必须 `docker compose up -d --build web`;改 `web/**` 因 bind-mount 直接刷新即生效。**pre-existing 没动**:本 PR 未触碰 `auth_refresh / auth_cookie_storage / warm_pool` 等已知 flake;ctest 跑时仍 `-E 'auth_refresh|auth_cookie_storage|warm_pool'` 过滤。 |
...
├── web/                       # 前端静态文件
│   ├── index.html             # 落地页（hero + 6 状态卡 + 适用场景 + 使用流程）— v1.3.3.9
│   ├── problems_list.html     # 题库列表页（筛选 + 分页，从 / 拆分）— v1.3.3.9
│   ├── problem.html
│   ├── login.html
│   ├── register.html
│   ├── profile.html
...
- [x] ★ Logout API（POST /api/v1/auth/logout；header-only + inline；`logout_handler` + `LogoutRequest` / `detail::parse_logout_request`；需 Bearer 鉴权（`require_authentication` 401），无 rate limit（SPEC §5.1）；**v1.3.3.8 ★ Phase 5 cookie-aware 修正**：refresh token 来源 = cookie `lc_refresh` 优先 → optional body `{refresh_token}` fallback（仅 dev `allow_body_fallback=true`）；body 缺失或空 → 200 `{logged_out:true, revoked:false}`（**不再 400** — pre-v1.3.3.8 强制 body field 把前端空 body 拒掉是隐藏 bug）；body present-malformed（非 string / ""）→ 400 INVALID_INPUT 但**仍下发 Set-Cookie Max-Age=0**（clear-cookie 提到 Bearer 鉴权后立即执行，任何 200/400 路径 cookie 都被清，关闭 XSS-steal 窗口）；`clear_refresh_cookie(res)` 提前到 Bearer 通过后立即调用；`revoke_refresh_token`（best-effort，never throws，TTL=剩余有效期，max_ttl_seconds cap）；theft defense：access token 的 claims.user_id 作为 `expected_user_id`，refresh sub 失配时 refused（log WARN），bob session 不受影响；anti-enumeration：malformed/expired/wrong-kind/access-as-refresh / 无 refresh presented → 同 200 + `{logged_out:true, revoked:false}`；幂等（store.revoke 重写 TTL）；tests/unit/test_auth_logout.cpp 30 用例（含 v1.3.3.8 新增 LogoutRevokesCookieRefresh / LogoutWithoutAnyTokenClearsCookie + 5 个 parse unit） + test_auth_cookie_storage.cpp LogoutClearsCookieWithMaxAgeZero 强化版 全通过 ~13s；不在本 PR：access token 黑名单 / `users.token_version` + Claims（留 v1.3.4））
...
- [x] ★ 题目列表页面（/problems_list.html，筛选 + 分页；从 / 拆分；落地页 index.html 只剩 hero + CTA + 6 状态卡 + 适用场景 + 使用流程）—— v1.2.24 / **v1.3.3.9 拆分**
- [x] ★ 题目详情 + 代码编辑器页面（双栏布局，集成 **Ace 1.32.7 SRI-pinned + textarea fallback**；Markdown 描述 + 样例 + 通过率 + 提交 + 轮询 + 草稿 + Ctrl/⌘+Enter 快捷键 + dark-mode 主题切换；v1.3.3.9 由 CodeMirror 5.65.16 迁移）—— v1.2.25
...
- [x] ★ 提交结果展示（AC/WA/TLE 状态 + 耗时/内存 + **失败时显示测试点** — 11 状态 icon + 耗时/内存 gauge（80% warning / ≥100% danger）+ 状态 specific help 文案 + 复制代码 + **复制判题摘要**（v1.3.3.9 一键复制 `[LiteCode] VERDICT · slug · lang · Xms · XKB · #id`）+ **通过测例进度** X/Y + 进度条（v1.3.3.9 桌面化 verdict hero block）+ 再次提交 + 失败时完整 `error_message` 详情折叠面板）—— v1.2.27 / **v1.3.3.9 桌面化升级**
- [x] ★ 异步判题轮询/SSE 客户端
- [x] ★ 提交历史标签页（刷题页下方）
...
- [x] ★ 审计日志测试（删题/改角色后查 audit-logs 验证写入）—— v1.2.64（e2e A36 系统化覆盖 audit-logs API 契约：权限 401/403 + 字段 id/action/admin_id/target_type/target_id/payload/created_at 齐全 + created_at 标准时间格式 + 组合筛选 action+target_type 同时生效 + 不存在的 action → items=0/total=0 + 分页 limit=1 vs limit=20 total 一致 + 默认 created_at DESC 排序 + 独立 fixture 全链路 改角色+创题+删题 后查 target_id 命中 ≥1 条；在 A17/A18/A20/A27/A35 各自孤立「有这条记录」之上做契约层覆盖。dry-run 20/0/30 exit 0，STRICT 20/30/0 exit 1）
- [x] ☆ 压测报告（5/10/20 人并发判题，验证 P95 < 5s）—— v1.2.65（`scripts/load_test.sh` 黑盒并发压测：复用 e2e 的 provision/submit/poll，提交 two-sum AC 解，每并发级起 N 个后台 worker 同时提交并轮询终态；采样 `submit_api_ms`(curl `%{time_total}`) + `e2e_ms`(提交→终态墙钟 `date +%s.%N`)，`sort -n` 取序位法算 p50/p95/p99；每级三断言 e2e P95 < 5000ms（§12.2）+ 提交 API p95 < 200ms + 错误数=0；`CONCURRENCY_LEVELS`/`P95_THRESHOLD_MS`/`LOAD_STRICT` 注入，缺栈宽松 skip / STRICT fail；跑完把环境快照 + 每级分位数表再生进 `docs/load-test-report.md`。本机 Docker down 未实测，报告结果段为 pending 占位，真实跑一次即覆盖）
- [x] ☆ 渗透测试（XSS / SQL 注入 / CSRF 扫描）—— v1.2.66（e2e A37–A39：8 条 SQLi payload × 3 反射面（login 用户名 /problems difficulty+tag /admin/audit-logs action）+ 5 条 XSS payload × 3 反射面（register username /auth/login 信封 Content-Type 结构化 /problems?tag 反射）+ 3 组 CSRF 探针（跨源 preflight 403+不回声 evil / 同源 204+精确 ACAO 回声 / refresh cookie HttpOnly+SameSite=Strict / 跨源 POST /auth/refresh 不带 cookie 因 SameSite 阻 401）；每条 SQLi/XSS 探针三断言 HTTP ≠ 500 + body 无 SQL 错误指纹（syntax error at or near / SQLSTATE / ORA- / unterminated quoted string 等 10+ 模式）+ 探针收尾用合法 admin 凭证登 200 验证 users 表未被破坏；A37a 16 login 探针另加「非 200」防认证绕过；A38b login 错误信封强校验 Content-Type=application/json + jq 可解析 + .code ∈ {INVALID_INPUT, UNAUTHORIZED, RATE_LIMITED, FORBIDDEN}（防「带外异常返回 text/html + SQL 异常体」））
- [x] △ 模糊测试（fuzzing）判题输入 —— v1.2.67（e2e A40–A43：`scripts/fuzz_judge.sh` 黑盒 fuzz，bash+curl+jq+python3 自带字节级 mutation 引擎替代 radamsa —— Python `random` + flip/insert/delete/overwrite/duplicate 五种 ops；A40 50 个 source_code mutation → POST /submissions 必须到达终态（ac|wa|ce|tle|mle|ole|re|pe|se 之一），零 5xx + 零 stuck-in-pending >30s + /health 200；A41 30 个 submission 字段边界 mutation（problem_id ∈ {0,-1,INT_MAX,2^63,-2^63,"abc",null,1.5,"0",INT_MIN} + language ∈ {"C++","cpp\n","c\x00","../cpp","\xc3\x28","a"*1000,null,"g++"} + code ∈ {空/64KB/NUL/0xff0xfe0xfd/嵌入 NUL}）零 5xx；A42 50 个 HTTP envelope mutation（25 /auth/login + 25 /auth/register，register 用 X-Forwarded-For 10.99.x.y 给每 probe 独立 IP bucket 避免 65s × 4 sleep —— 同 v1.2.63 A26a 思路）；A43 20 burst mutation + 中途 /health 探针 + 终态 /health 200 不变式。429 RATE_LIMITED 不算 fail（rate-limit 主动保护预期行为），400 INVALID_INPUT 不算 fail（validator 在工作）。e2e wrapper 委托 fuzz_judge.sh，末行 `FUZZ_RESULT PASS=N FAIL=N SKIP=N` 反向汇入主计数器。dry-run 通过预期；缺栈宽松 skip / STRICT fail）
...
- [x] △ 性能 Profile（`perf` / `flamegraph` 跑一次判题热路径）—— v1.2.74（`scripts/perf_profile.sh` 678 行黑盒 profile 工具，三段：A) HTTP 面分阶段 timing 拆解（curl -w 6 阶段模板 `time_namelookup/connect/appconnect/pretransfer/starttransfer/total`，对 /health /problems /auth/login /api/v1/metrics 4 个关键 endpoint 各 N 次采样取分位数，硬断言 SPEC §12.2 阈值 50ms/200ms/200ms/200ms；B) Prometheus histogram 采样（前后两次 scrape `litecode_judge_duration_seconds` 算 `_count/_sum` 增量 + `_bucket` 线性插值反推 p50/p95/p99，类似客户端 histogram_quantile 实现）；C) Linux `perf record -F 99 -g -p $PID -- sleep 30` + `docker cp` + `perf script | stackcollapse-perf.pl | flamegraph.pl` 生成 `docs/perf-flamegraph.svg`——只 Linux 容器 + perf/FlameGraph 工具链齐全时跑，其他平台 SKIP；`scripts/lint.sh perf_profile` 22 项关键环节点 + 行数 ≥ 250 + bash -n + 可选工具探测；`scripts/e2e_acceptance.sh` A45 三层断言（A45a bash -n + 6 项关键环节点 + 行数 / A45b runbook 含 SPEC §12.2 阈值引用 / A45c 委托运行 + `PROFILE_RESULT PASS=N FAIL=N SKIP=N` 反向汇入主计数器，v1.2.67 FUZZ_RESULT / v1.2.72 DRILL_RESULT 同款设计）；`docs/performance-profile.md` 报告（运行时再生结果段，沿用 v1.2.65 load-test-report.md 模板）；`docs/runbooks/performance-profile.md` 完整 runbook（§3 SPEC §12.2 阈值表 + §4 Phase A 原理 + §5 Phase B histogram 反推 + §6 Phase C perf+flamegraph 跑法 + §7 一键操作 + §8 故障排查 + §9 与 load_test / metrics / alerting 关系 + §10 follow-up 含 `litecode_http_request_duration_seconds` histogram 占位 + §11 版本演进）；PROFILE_STRICT=1 把缺前置升级 FAIL；零 src/* 改动；SPEC.md 短版 line 150 + 长版 line 1137 两镜像同翻）
...
- 每月 1 次 restore drill 到测试环境（`scripts/restore_drill.sh`），验证 backup 可用 — v1.2.72；详见 [`docs/runbooks/monthly-restore-drill.md`](../docs/runbooks/monthly-restore-drill.md)
