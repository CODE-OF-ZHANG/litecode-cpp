...
- [x] ☆ Special Judge 框架（v1.3.1 P0 闭环：admin 3 端点 + judge.sh 接线 + 公共 detail 透 judge_type / has_special_judge + 测试 + e2e A48 + runbook；P1 judge_type 扩展 / P2 Interactive 推 v1.4；`commit 2ea2934`）
- [x] ☆ judge_type 扩展 ignore_case / ignore_all_whitespace（v1.3.1+：V011 ENUM 扩展 + compare.sh 新函数 + judge.sh case 分支 + admin 双端 validator + 单测 + e2e A49 + README 同步；`commit 2ea2934`）
