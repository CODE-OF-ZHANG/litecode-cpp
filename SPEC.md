...
| v1.2.19 | 2026-07-03 | Phase 5 开篇：实现 §11 Phase 5 第一项 ★ `前端框架（公共导航栏 + api.js 封装 + 统一错误处理 + 401 自动跳转登录）`；新增 `web/js/api.js`（litecode.api = 单一 fetch 入口：`{get,post,put,delete,patch,rawFetch,sse}` 全部 return `{ok,status,data,request_id,raw}` envelope-aware；`LitecodeApiError` 错误类型带 `{status,code,message,details,request_id}` 五字段贴 SPEC §5.7 envelope；`litecode.auth.{getAccessToken,getRefreshToken,setTokens,clear,fetchProfile,login,register,logout}` + `isLoggedIn` / `isAdmin` + `onUnauthorized` / `onAuthChanged` listener；access_token + refresh_token 暂存 sessionStorage（双存策略；SPEC §6.3 要求 access 内存 + refresh HttpOnly cookie — 这需要后端先加 Set-Cookie 响应 + /refresh 兼容读 cookie，Phase 5★前端框架不阻塞该路径，TODO 写在 api.js 末尾 + entry-level 注释）；`forceSignOut(reason)` 单点入口：清 sessionStorage + emit `litecode:api-unauthorized` event + `setTimeout(50ms)` 让 listener 跑完再 `window.location.href = '/login.html?next=…'`；401 → `fetchWithAutoRefresh()` 单飞 refresh-once mutex（`inflightRefresh` 同一毫秒内的并发 401 共享同一个 refresh promise，N 个 401 只触发 1 次 /auth/refresh 不会撞上后端的 5/分/IP 限流）→ 200 → replay 原请求；refresh 自己失败 → forceSignOut；`/auth/refresh` 与 `noRetryOn401:true` 路径不进入 retry 循环防递归；CustomEvent surface 给 app.js 解耦 toast：`document.addEventListener('litecode:api-error', …)` + `document.addEventListener('litecode:api-unauthorized', …)` + `document.addEventListener('litecode:auth-changed', …)`；超时 20s 默认（`SUBMIT_TIMEOUT_MS=30s` 留给 future async submission）经 `AbortController` 主动 abort 报 typed `INTERNAL_ERROR` envelope）；不暴露 native fetch 给 99% 调用方，只 `rawFetch` / `sse` 给流式/sse 逃生；新增 `web/js/app.js`（litecode.boot.shell({title, guestOnly, requireAdmin}) 单点入口：theme-before-paint 防止浅色闪一下再切深色 + nav eager-render from cached user + 后台 fetchProfile → auth-changed listener 重渲 nav + 状态↔管理员菜单 visibility；`litecode.nav.{mount,user}` + `litecode.theme.{get,set,toggle}` + `litecode.toast.{success,error,warn,info}` + `litecode.guard.{requireAuth,requireAdmin,requireGuest}`；`requireAuth` 守卫 → 未登录 `replace('/login.html?next=…')` 后返回永不 resolve 的 promise 让 caller 不再读 DOM；`requireAdmin` 检查 role=admin 否则 `toast + replace('/index.html')`；toast 自动接 `litecode:api-error` → 按 `err.status` 映射 kind（401 不会 toast 因为已经在 redirect；/logout 触发的 unauthorized 也不会 toast 因为是用户主动行为）；`litecode.markdown.{prewarm,renderSafe,renderSafeSync}` lazy 加载 `marked@12.0.2 + dompurify@3.1.6` from cdn.jsdelivr.net，SRI hash 留空待后续 deploy 工具钉死（CSP meta + Caddy header 都允许该域），同步失败 fallback 成 plain-text escape 不会把 server-supplied Markdown 当 HTML 注入；`toggleTheme()` 持久化到 localStorage 的 `litecode:theme`，无显式选择时跟 `prefers-color-scheme` 媒体查询）；新增 `web/css/style.css`（基础样式单文件）：CSS 变量 token 含 light/dark 两套（`html.dark` 类切换；无存储时 `prefers-color-scheme` 兜底，通过 `data-theme-chosen` 在 html 上屏蔽）；`.lc-container` / `.lc-stack` / `.lc-row` / `.lc-card` layout primitives；`.lc-btn--{primary,ghost,danger}` 三态 + `.lc-icon-btn` 主题切换按钮；form tokens `.lc-input` + `.lc-form-{row,label,help,error,section}` 含 password help text；`.lc-nav` 顶栏 sticky + brand + links + user-cluster（主题切换按钮 + 登录/注册 或 avatar▾）；`.lc-menu` popover（`.lc-menu-item` + `.lc-menu-sep` + `.lc-menu-item--danger` 退出登录）；`.lc-toasts` 固定右下角 1000z-index + 入场动画 + auto-dismiss 4s + hover 取消 + 4 kind 彩色左边框（success/error/warn/info）；`.lc-pill` 三难度配色 easy/medium/hard + admin pill；`.lc-status--{ac,wa,tle,mle,re,ole,pe,ce,se,pending,running}` 11 个判题状态 badge；`.lc-markdown` 给 SPEC §6.3 A32 渲染留 prose style；media `(prefers-reduced-motion: reduce)` 全 0.01ms + `<=768px` 折叠 nav / forms + `<=480px` nav-links 横向滚动；`*-visible` focus ring 用 `--lc-color-focus-ring` 透明版保 a11y；`index.html` / `login.html` / `register.html` / `profile.html` 四壳：`[data-nav]` slot + `<meta http-equiv="Content-Security-Policy">` meta tag + `<link rel="stylesheet" href="/css/style.css">` + `<script src="/js/api.js" defer>` + `<script src="/js/app.js" defer>` + `DOMContentLoaded` 块调 `litecode.boot.shell(...)`；`index.html` 演示页：题库页骨架含两个 demo 按钮（测试 /health、列前 5 题）+ `litecode:api-error` 事件 → toast 自动弹；`login.html`：`guestOnly:true` + `litecode.api.auth.login()` 把 `{RATE_LIMITED, INVALID_INPUT, UNAUTHORIZED, INTERNAL_ERROR}` 四路分支映错误文案，成功后 `replace(next)` 回跳；`register.html`：同样的 guestOnly + 后端 `validate_password_strength` 同步到前端（`USERNAME_RE` 3-50 字符 / `EMAIL_RE` / `PW_RE` 8-72 + 字母数字各 1 + 与确认密码匹配），`err.details.field` 决定 focus 哪个 field，成功后 `replace('/index.html')`；`profile.html`：`litecode.guard.requireAuth()` 入口 + 守卫失败 `replace('/login.html?next=…')`，成功走 `litecode.api.auth.fetchProfile()` + 渲 user block + role pill（管理员蓝色 `.lc-pill--admin`）+ 注册日期；Phase 5 框架不引入静态分析或 mocha 测试（SPEC §11 不要求单元测前端 — 后续 e2e_acceptance.sh 由 Playwright/Puppeteer 验），静态语法校验 `node -c web/js/api.js` + `node -c web/js/app.js` 两文件语法 OK；Phase 5 后续条目（CSP 头 + SRI 配 file/header、Login/Register 完整 feature、题库/详情/编辑器/草稿、Submit/SSE/Dark-mode/Mobile-Responsive 等）由各自 commit 接力 |（SPEC §4.3 + §15.4 + §7.1.b `special` 分支 + §11 Phase 4 末项；标 v1.3 提前到 Phase 4 收尾交付）；新增 `db/migrations/V010__add_problem_special_judges.sql`（CREATE TABLE IF NOT EXISTS `problem_special_judges` problem_id PK FK problems ON DELETE CASCADE / source MEDIUMTEXT NOT NULL / language ENUM('cpp') NOT NULL DEFAULT 'cpp' / created_at + updated_at DATETIME CURRENT_TIMESTAMP + ON UPDATE；INSERT IGNORE schema_migrations 幂等；端到端本地 MySQL 8.0.41 应用 V001-V010 顺序 OK）；新增 `src/db/special_judge_repo.h`（namespace `special_judge_repo`，header-only + inline；`SpecialJudgeRow`（problem_id / source / language / created_at / updated_at 四个 DATE_FORMAT'd 文本字段防 mysql-connector 9.x packed-binary 坑）/ `SpecialJudgeRepoError` / `SpecialJudgeNotFoundError` 两级异常继承 std::runtime_error；`validate_source` 1..16MB + `validate_language` 仅 'cpp'；`detail` 私有命名空间避跨 repo ODR / `kSpecialJudgeSelectColumns` 常量集中列序；`find_by_problem_id` → `std::optional<SpecialJudgeRow>`，nullopt = 题未挂 SPJ（judge.sh 把 special 点判 WA 而非 SE，给 operator 留迭代空间）/ `exists_for_problem` boolean hot-path helper / `upsert` 走 INSERT ... ON DUPLICATE KEY UPDATE（FK violation 1452 → typed exception 让路由层映 400）/ `remove_by_problem_id` 幂等返 bool / problem_id<=0 / 空 source / 非 cpp language 三道防御抛 `SpecialJudgeRepoError`）；`judge/lib/spj.sh` 新增 SPJ 编译 + 调度 helpers（`compile_spj <src> <bin>` g++ 用与 user 提交相同的 §7.1 安全 8 标志 + 独立 10s 防编译炸弹 + 错误文件落 `bin.err`；`run_spj <bin> <in> <exp> <act>` 三参调用 SPJ 二进制：rc=0 AC / rc=1 WA / rc=124 TLE / rc=137 MLE / other SE；`spj_stdout_for_info` + `spj_err_for_info` 把截断 256B / 4KB 的 stdout+stderr 折进 `case_results.info`；`compare_special_with <bin> <out> <exp> <in>` 入口把上面穿起来）；`judge/lib/compare.sh::compare_special` 重写为薄壳（`LITECODE_SPJ_BIN` 环境变量置了就 `compare_special_with` 调用 SPJ，否则返 WA — 防 operator 在题还没挂 SPJ 时被 SE 刷屏）；`judge.sh` Section B 新增 `SPECIAL_JUDGE_SOURCE_RAW` / `SPECIAL_JUDGE_LANGUAGE_RAW` / `SPECIAL_JUDGE_ENABLED` 三变量（jq 解析 + null 归一为空字符串）；Section C 之前任一 case `judge_type=special` 触发一次性 `compile_spj` 把 SPJ 编到 `JUDGE_TMP/spj_bin`，rc=0 且 binary 可执行则 `export LITECODE_SPJ_BIN` 否则清空；Section D 的 `special)` 分支三态：①SPJ 编译失败 → 每个 special 点 emit SE + stderr 折 info + 整 submission SE；②题未挂 SPJ → 每个 special 点判 WA + info "no special judge configured"；③SPJ 编译成功 → 调 `compare_special_with` SPJ 三参：rc=0 AC, rc=1 WA, other（含非法退出码）→ SE+info "spj exit=N: <stdout>"），整 SPECIAL_JUDGE path 用 `set +e` 包裹精准捕 rc）；`src/judge/judge_scheduler.h` 给 `JudgeTask` 加 `spj_source` + `spj_language` 两字段（spj_source 空 = 题未挂 SPJ = judge.sh 走 WA 回退；spj_language 留作未来 python/java/... 扩展钩子）；`write_task_json` 把 spj_source / spj_language 用 `null` JSON sentinel（task.json schema 兼容：未挂 SPJ 时下游 jq 仍能解析为空串）；`src/routes/submission_routes.h::create_submission_handler` 在原来 6 步管道后多 6b 步：尝试 `special_judge_repo::find_by_problem_id(pool, *problem_id)`；读到 → 设 `task.spj_source` + `task.spj_language` + LOG_INFO "special judge attached"（含 source_bytes / language）；读异常 / 题无 SPJ → LOG_WARN（**不是 500**；submission row 仍然有效，judge.sh 的 SPJ 路径会把所有 special 点判 WA，operator 在 dashboard 一眼看见，没必要为 SPJ 暂时不可用把整个 POST 顶到 500）；admin_problem_routes / admin_bulk_import_routes / problem_routes / tag_routes 完全未动 — SPJ 框架只新增写路径 storage + 读路径 dispatcher，admin 上传 SPJ 的 REST endpoint（POST /api/v1/admin/problems/:slug/special-judge）属后续 commit；新增 `tests/unit/test_special_judge.cpp` 14 用例全通过 ~1.3s：(a) 纯单测 5 用例 — kMinSpjSourceLen=1 + kMaxSpjSourceLen=16MB + kSpjLanguageCxx 字符串 pin / `validate_source` 空拒 + 1 字 + MAX OK + MAX+1 拒 / `validate_language` cpp 接 + c/py/python/java/C++ 拒 / `SpecialJudgeRow` defaults / `SpecialJudgeRepoError` ↔ `SpecialJudgeNotFoundError` 两级继承 + (b) MySQL 集成测试 9 用例（V010 schema probe，缺表 SKIP）— FindByProblemIdMissingReturnsNullopt / ExistsForProblemMirrorsFind（false → upsert → true）/ UpsertHappyPathThenFindRoundTrips（byte-for-byte round-trip + created_at/updated_at 非空）/ UpsertReplacesExistingSource（v1 / v2 + sleep(1s) 让 CURRENT_TIMESTAMP 推进）/ UpsertRejectsBadLanguage（java / 空都抛 + exists_for_problem 仍是 false）/ UpsertForeignKeyViolationSurfacesTypedError（problem_id=9_999_999 抛 SpecialJudgeRepoError）/ RemoveByProblemIdIdempotent（false × 3 + true × 1 + false × 1）/ RemoveByProblemIdRejectsNonPositiveId（0 / -1 抛）/ FkCascadeHardDeleteProblemWipesSpjRow（hard-delete parent → SPJ 因 ON DELETE CASCADE 也消失 + tracker 清掉该 id 防 RAII 重复清理）；`tests/CMakeLists.txt` 新增 `test_special_judge` target（mysql::concpp + OpenSSL + GTest::gtest_main，复用 test_audit_log 同 link 栈，MSVC /bigobj 不需要 — 只过 special_judge_repo 单 repo header，无跨多 repo ODR 风险）+ add_test；test_submission 同步加 /bigobj（特殊判定 repo 多过 2^16 sections 上限 / 防御性 — 实际未触发但保留作 v1.3 admin SPJ endpoint 落地时再叠加 repository 时的一道护栏）；judge.sh + lib/spj.sh + lib/compare.sh 三件 `bash -n` 语法 OK；Phase 4 全章收尾；Phase 4 剩余项（SSE 推送 v1.2.17 + Special Judge 框架本条）已全部交付 |
