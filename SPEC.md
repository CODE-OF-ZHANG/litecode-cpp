...
| v1.2.30 | 2026-07-06 | Phase 5 续：落地 §11 Phase 5 第十二项 ★ `个人主页（/profile/:username，做题统计）`（F6/F7 + A15）；前序 v1.2.19 仅交付一个最小 `/auth/profile` 外壳（头像 + 用户名 + 角色 pill + 注册日期，无任何做题统计），本 commit 升级到"完整可生产"版本；**数据来源决策**：SPEC §5.4 的专用统计端点 `GET /api/v1/stats/profile/:username` 属 **Phase 6**（`src/routes/stats_routes.h` 仍是 0 字节），本 Phase 5 前端页不阻塞该后端项——做题统计全部**在客户端聚合**自两个已发布的 API：`GET /api/v1/submissions`（v1.2.16，非 admin 后端强制 `user_id = self`，含 `{items,total}` + 每条 `{id,problem_id,language,status,time_used,memory_used,created_at,finished_at}`）+ `GET /api/v1/problems`（v1.2.6，活题目录，供 "已解决 / 总题数" 分母、每难度题目总数、以及 `problem_id → {slug,title,difficulty}` 解析给最近提交列表链接）；因 `/submissions` 对非 admin 是 self-only，本页只能算**登录用户自己**的统计，查看他人 `?username=other` 时优雅提示指向 Phase 6 端点（`showForeign()`），自己主页完整可用；Phase 6 stats API 落地后只需把 `loadStats()` 的两次分页 fetch 换成一次 `GET /api/v1/stats/profile/:username`，render 层已接受纯 stats 对象；**URL 约定**：静态托管无 path 参数，`:username` 以 `?username=` 承载（同 problem.html 的 `?slug=`）；**`web/profile.html` 重写**（128 → 约 400 行，inline `<script>` 单一 IIFE + `'use strict'` 17103 字符）：(a) **`litecode.guard.requireAuth()` 守卫**（未登录 → `/login.html?next=…` 重定向，promise 永不 resolve）→ `fetchProfile()` 拿 `{username,role,created_at,last_login}` → `resolveUsername(search, currentUsername)` 判 self/foreign；(b) **四态切换** `showState()`：loading (spinner) / error (+重试按钮，重试走 `loadStats()` 而非整页 reload) / foreign (他人主页提示卡) / content (统计正文)；(c) **`fetchAllPages(basePath, extraQuery)`** 泛型分页器：`limit=100&offset=N` 循环拉取直到 `items.length >= total` 或触及 `MAX_PAGES=20` 硬上限（≤ 2000 行/资源，`capped` 标志驱动 "统计基于最近 N 条提交" 注脚），`noRetryOn401:true` 让 401 走 api.js 全局 `forceSignOut`；两资源 `Promise.all` 并行拉取；(d) **`computeStats(subs, metaMap)`** 纯函数聚合：`totalSubmissions` / `acSubmissions` / `acceptanceRate = ac/total`（0 分母安全，无 NaN）/ `solvedCount`（distinct AC problem_id）/ `attemptedCount`（distinct 任意 problem_id）/ `byStatus`（11 状态计数）/ `byDifficultySolved`（easy/medium/hard/unknown——软删题的历史 AC 计入 solvedCount 但归 unknown 桶）；(e) **`buildProblemMeta(items)`** → `{map: id(string)→{slug,title,difficulty}, diffTotals: {easy,medium,hard}}`；(f) **渲染**：header（大头像 `.lc-avatar--lg` + 用户名 + 角色 pill + 注册日期 + 相对格式上次登录）+ 4 张 stat card（已解决/总题数 accent + 覆盖率、通过率 + AC/提交、总提交数、尝试过的题目）+ 难度分布 bar（已解决 / 该难度题目总数，easy 绿 / medium 橙 / hard 红）+ 提交状态分布 bar（按 STATUS_ORDER，AC 绿其余灰，denom=最大单状态计数，0 提交时显示空提示）+ 最近提交 list（复用 v1.2.29 `.lc-history-row` + `.lc-status--*` token；有 slug 的题目链到 `/problem.html?slug=`，软删题回退 `题目 #id`）；(g) **XSS 防御** 全部走 `el()` + `textContent`/`setAttribute`（无 `innerHTML` 用户数据路径），与 v1.2.24 escapeText / v1.2.20 DOMPurify 同源契约；(h) **辅助** `formatPercent`（0..1 → "NN.N%"）/ `pct`（0 分母 → 0）/ `formatProfileTime`（复用 v1.2.26 相对/绝对时间词汇：刚刚 / N 分钟前 / N 小时前 / 绝对日期）；(i) **测试钩子** IIFE 末尾 `if (typeof __lcTest !== 'undefined')` 暴露 resolveUsername / formatPercent / formatProfileTime / buildProblemMeta / computeStats / pct / STATUS_LABEL / DIFF_LABEL；(j) 移除 v1.2.19 的 markdown.js 依赖（本页不渲染 Markdown），脚本回到 csp→api→app 三段；**`web/css/style.css` §16 新增章节** "Profile page — 做题统计"（约 27 新 selector）：`.lc-profile-head` flex + `.lc-avatar--lg`（64px）+ `.lc-profile-head__body/__name/__meta`；`.lc-stat-grid` auto-fit minmax(150px,1fr) grid + `.lc-stat-card` + `__value`（28px tabular-nums）+ `--accent`（link 色）+ `__label` + `__sub`；`.lc-section-title`；`.lc-breakdown` flex column + `.lc-bar-row`（88px 1fr 96px grid）+ `__label` + `.lc-bar-track`（10px 圆角 surface-2）+ `.lc-bar-fill`（300ms width 缓动 min-width 2px）+ `--easy`(#16a34a)/`--medium`(#d97706)/`--hard`(#dc2626)/`--ac`(success)/`--muted`(text-2) + `.lc-bar-row__count`（tabular-nums 右对齐）；`.lc-profile-notice` 虚线卡；600px 断点收缩 bar-row 网格 + stat value 字号；CSS 大括号配平 364/364（v1.2.29 335/335 + +29）；**`web/test/profile.test.js` 新增 33 用例**（vm sandbox 拉 profile.html inline IIFE 走 `__lcTest` 桥；sandbox 加 `addEventListener` no-op 让 IIFE 顶层 `window.addEventListener('DOMContentLoaded')` 跑通）：group 1 (6) resolveUsername 空/自身/大小写不敏感/他人/无当前用户/空白 trim；group 2 (5) pct 0分母+3/4 + formatPercent 0/1/舍入；group 3 (5) buildProblemMeta 每难度总数 + string-key map + 未知 id 缺席 + 空目录；group 4 (12) computeStats 总数/AC数/distinct solved/distinct attempted/通过率/per-status/per-difficulty + 空历史无 NaN + 软删题 AC 归 unknown + 缺 status 默认 pending；group 5 (3) formatProfileTime 空/不可解析/绝对日期形态；33/33 PASS `node web/test/profile.test.js`；**静态校验**：`node -c` csp/api/app/markdown 四文件 OK + profile.html inline IIFE `new Function` 解析 17103 字符 OK + profile 33/33 + HTML static check 11/11（profile.html CSP meta 与 csp.js canonical 字节级一致 + 脚本序 csp→api→app）+ 回归 history 45/45 + sse-parser 49/49 全 PASS；**无新增后端 / DB / 测试二进制 / 路由变更**（仅前端静态资源 + 1 个测试 JS；`stats_routes.h` 仍留给 Phase 6）；Phase 5 后续条目（排行榜页 / 管理后台 5 页 / 前端权限拦截 / 深色模式 / 移动端响应式）由各自 commit 接力 |
